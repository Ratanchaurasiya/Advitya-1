...
/**
 * Business Health Showcase Component for Evoniq ERP
 *
 * Provides real-time executive visibility with composite & domain-specific
 * health gauges (Lead, Sales, Revenue), core KPI tracking, and automated insights.
 */
...
/* ──────────────────────────────────────
   SVG Health Gauge Component
   ────────────────────────────────────── */
function HealthGauge({ score, label, color, size = 'large' }) {
  // Arc calculation: full arc spans from angle 150° to 390° (240° sweep)
  ...
  const filledAngle = startAngle + (sweepAngle * Math.min(Math.max(score, 0), 100)) / 100;

  const toRad = (deg) => (deg * Math.PI) / 180;
  const polarToCart = (centerX, centerY, r, angle) => ({
    x: centerX + r * Math.cos(toRad(angle)),
    y: centerY + r * Math.sin(toRad(angle)),
  });
  ...
  const filledSweep = (sweepAngle * Math.min(Math.max(score, 0), 100)) / 100;
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative">
        <svg
          viewBox={viewBox}
          className={size === 'large' ? 'w-44 h-[104px]' : 'w-28 h-[72px]'}
        >
          {/* Background arc */}
          <path
            d={bgPath}
        ...
        <div className="absolute inset-x-0 bottom-0 text-center">
          ...
          <p
            ...
            } font-semibold uppercase tracking-wider ${statusColor}`}
          >
            {statusLabel}
          </p>
        </div>
...
/* ──────────────────────────────────────
   Main Business Health Section Component
   ────────────────────────────────────── */
export default function BusinessHealthSection() {
  const subGauges = [
    {
      ...
      color: 'blue',
      description:
        'Measures lead capture velocity, source diversity, and assignment efficiency.',
      metrics: [
    ...
    },
    {
      ...
      color: 'violet',
      description:
        'Tracks pipeline progression, follow-up discipline, and site visit conversion.',
      metrics: [
    ...
    },
    {
      ...
      color: 'orange',
      description:
        'Monitors booking value, collection velocity, and revenue forecast accuracy.',
      metrics: [
  ...
  return (
    <section
      id="business-health"
      className="relative py-20 lg:py-28 bg-gradient-to-b from-slate-50 to-white"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-slate-600 mt-4 leading-relaxed">
            Evoniq ERP computes a real-time Business Health Score by analyzing
            lead flow efficiency, sales pipeline velocity, and revenue collection
            patterns — giving management instant, data-driven visibility across
            all four ERP modules.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="px-6 sm:px-8 py-4 bg-slate-900 flex items-center justify-between">
            ...
            <div className="hidden sm:flex items-center gap-2 text-xs text-slate-400">
              ...
              <span>Evoniq ERP Real-time analytics engine</span>
            </div>
          </div>

          <div className="p-6 sm:p-8 lg:p-10">
            {/* 4-Column Layout: Composite + 3 Sub-Gauges */}
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 lg:gap-6 items-center mb-10">
              {/* Overall Health Gauge (Large, Emerald) */}
              <motion.div
                initial="hidden"
                whileInView="visible"
                viewport={{ once: true }}
              ...
              >
                {subGauges.map((sub, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col items-center bg-white rounded-2xl border border-slate-200/60 p-5 hover:shadow-md hover:border-slate-300 transition-all"
                  >
                    <HealthGauge
                      score={sub.score}
                      label={sub.label}
        ...
        >
          {benefitChips.map((item, idx) => (
            <span
              key={idx}
              className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-600 bg-white border border-slate-200 px-3.5 py-2 rounded-full shadow-xs hover:border-slate-300 transition-colors"
            >
